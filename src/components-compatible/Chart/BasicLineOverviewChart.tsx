...
import { NormalizeGraphQueryContainerResult } from '@/utils/dbquery';
import { FormattedMessage } from 'umi';

interface IBasicLineOverviewChartProps {
  ResourceId: string;
  XAxisKey: string;
  YAxisKey: string;
  normalizeCharData: (values: Array<string>[]) => any[];
...
export function BasicLineOverviewChart(props: IBasicLineOverviewChartProps) {
  const {
    ResourceId,
    QueryColumns,
    XAxisKey,
  ...
  const [ResultValues, ChangeResultValues] = useState<Array<string>[]>([]);
    ...
    function getChartRawData() {
      GetChartRawDataRecord(ResourceId, 'hour', RecentCount, columnsRef.current)
        .then((response: AxiosResponse) => {
          const series = NormalizeGraphQueryContainerResult(response);
          if (series) {
            ChangeResultValues(series.values);
          }
        })
        .finally(() => {
  ...
  const chartData = useMemo(() => {
    return normalizeCharData(ResultValues);
